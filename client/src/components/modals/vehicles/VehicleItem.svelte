<script lang="ts">
import { afterUpdate } from 'svelte';

import VehicleTypeIcon from '$components/common/VehicleTypeIcon.svelte';
import MavlinkIdEdit from '$components/modals/vehicles/MavlinkIdEdit.svelte';

import { type VehicleDescription, is_vehicle_online } from "$bindings/vehicles";
import { vehicleStatuses, vehicleDescriptions, vehicleTypes, selectedVecicleID } from "$stores/vehicles";

import { i18n } from "$stores/i18n";

export let editingVehicleID = ""

export let vehicle: VehicleDescription
export let changed: boolean = false

let vehicleCopy: VehicleDescription = vehicle

$: status = $vehicleStatuses.get(vehicle.id || "")
$: is_online = is_vehicle_online(status)

afterUpdate(async () => {
    if (editingVehicleID !== vehicle.id) {
        vehicleCopy = vehicle;
    } else {
        changed = JSON.stringify(vehicleCopy) !== JSON.stringify(vehicle);
    }
});

</script>

<div class="collapse collapse-arrow bg-base-200">
    <input type="radio" checked={ editingVehicleID === vehicle.id } name="communication-vehicles-accordion"
        on:change={() => { editingVehicleID = vehicle.id || "" }}/>
    <div class="collapse-title flex flex-row gap-4 w-full content-center">
        <!-- VEHICLE INDICATOR -->
        <div class="indicator">
            <span class={"indicator-item badge badge-xs " +
                (is_online ? "bg-success" : "bg-neutral-content")} >
            </span>
            <VehicleTypeIcon vehicleType={vehicle.vehicle_type} />
        </div>

        <!-- VEHICLE TITLE -->
        <h1 class="font-medium ml-2 my-2">{vehicle.name}</h1>

        <div class="grow"/>

        <!-- SELECT VECICLE -->
        <button class={ "btn btn-sm z-[1] " + ($selectedVecicleID === vehicle.id ? "btn-outline btn-disabled" : "btn-ghost") }
            on:click={() => { selectedVecicleID.set(vehicle.id || "") }}>
            { $selectedVecicleID === vehicle.id ? $i18n.t("Selected") : $i18n.t("Select") }
        </button>

        <div class="join btn-sm p-0 z-[1]">
        </div>
    </div>
    <div class="collapse-content gap-8">
        <div class="grid grid-cols-2 gap-2">
            <!-- NAME -->
            <h1 class="font-medium my-2 w-full">{ $i18n.t("Name") }</h1>
            <input type="text" placeholder={ $i18n.t("Enter name here") } class="input w-full" disabled={is_online}
                bind:value={vehicleCopy.name}/>

            <!-- VEHICLE TYPE -->
            <h1 class="font-medium my-2 w-full">{ $i18n.t("Type") }</h1>
            <select class="select w-full" bind:value={vehicleCopy.vehicle_type} disabled={is_online}>
                {#each vehicleTypes as type, i}
                <option value={type} disabled={i === 0}> { $i18n.t(type) }</option>
                {/each}
            </select>

            <!-- TODO: VEHICLE FEATURES -->

            <!-- PROTOCOL ID -->
            {#if vehicleCopy.protocol_id.MavlinkId}
                <MavlinkIdEdit bind:protocol_id={vehicleCopy.protocol_id.MavlinkId} disabled={is_online}/>
            {/if}
        </div>

        <div class="w-full btn-sm mt-4 flex">
            <button disabled={is_online} class="btn btn-sm btn-wide btn-outline btn-secondary px-1 ml-2"
                on:click={() => { vehicleDescriptions.removeVehicle(vehicle.id || "") }}>
                { $i18n.t("Remove") }
            </button>

            <div class="grow"/>

            <div class="join btn-sm p-0">
                <button disabled={!changed} class="btn btn-sm btn-wide btn-primary join-item px-1 ml-2"
                    on:click={()=> { vehicleCopy = vehicle }}>
                    { $i18n.t("Discard") }
                </button>
                <button disabled={!changed} class="btn btn-sm btn-wide btn-accent join-item px-1 ml-2"
                    on:click={ async () => { vehicle = await vehicleDescriptions.saveVehicle(vehicleCopy) || vehicle }}>
                    { $i18n.t("Save") }
                </button>
            </div>
        </div>
    </div>
</div>