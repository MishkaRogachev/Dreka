...
let descriptionCopy: LinkDescription = link.description

afterUpdate(async () => {
    if (selectedLinkId !== link.description.id) {
        descriptionCopy = link.description;
    } else {
...
<div class={"collapse collapse-arrow bg-base-200"}>
    ...
    <div class="collapse-content gap-8">
        <div class="grid grid-cols-2 gap-2">
            ...
            <input type="text" placeholder={ $i18n.t("Enter name her") } class="input w-full"
        ...
        <div class="w-full btn-sm mt-4 flex">
            ...
            <div class="join btn-sm p-0">
                <button disabled={!changed} class="btn btn-sm btn-wide btn-primary join-item px-1 ml-2"
                    on:click={()=> { descriptionCopy = link.description }}>
                    { $i18n.t("Discard") }
                </button>
