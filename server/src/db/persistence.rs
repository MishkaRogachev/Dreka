...
impl Persistence {
    ...
    pub async fn create<D>(&self, table: &str, data: &D) -> Result<D, surrealdb::Error>
    where D: serde::ser::Serialize + for<'de> serde::Deserialize<'de> {
        let mut records: Vec<D> = self.db.create(table).content(data).await?;
        Ok(records.pop().unwrap())
    }

    pub async fn update<D>(&self, table: &str, data: &D) -> Result<D, surrealdb::Error>
    where D: serde::ser::Serialize + for<'de> serde::Deserialize<'de> {
        let mut records: Vec<D> = self.db.update(table).merge(data).await?;
        Ok(records.pop().unwrap())
    }
