...
use mavlink::{MavHeader, common::{MavMessage, ATTITUDE_DATA, ALTITUDE_DATA, VFR_HUD_DATA, GLOBAL_POSITION_INT_DATA, GPS_RAW_INT_DATA}};
...
pub struct TelemetryHandler {
    ...
    mav_altitude_data: HashMap<u8, AltitudeData>,
...
impl TelemetryHandler {
    ...
    pub async fn handle_message(&mut self, header: &MavHeader, msg: &MavMessage) {
        match msg {
            ...
            MavMessage::ALTITUDE(altitude) =>
                self.handle_altitude(header.system_id, altitude).await,
    ...
    pub async fn handle_altitude(&mut self, mav_id: u8, altitude: &ALTITUDE_DATA) {
        let mut altitude_data = self.altitude_data_for_mav(&mav_id);

        altitude_data.reference_altitude = altitude.altitude_monotonic;
        altitude_data.altitude_amsl = altitude.altitude_amsl;
        altitude_data.altitude_relative = altitude.altitude_relative;
        altitude_data.altitude_terrain = altitude.altitude_terrain;
        altitude_data.bottom_clearance = altitude.bottom_clearance;
        altitude_data.timestamp = chrono::prelude::Utc::now().timestamp_millis();

        self.set_altitude_data_for_mav(mav_id, altitude_data).await;
    }
    ...
    async fn set_flight_data_for_mav(&mut self, mav_id: u8, flight_data: FlightData) {
        // TODO:  get id from vehicle
    ...
    fn altitude_data_for_mav(&self, mav_id: &u8) -> AltitudeData {
        match self.mav_altitude_data.get(mav_id) {
            Some(altitude_data) => altitude_data.to_owned(),
            None => AltitudeData::default(),
        }
    }

    async fn set_altitude_data_for_mav(&mut self, mav_id: u8, altitude_data: AltitudeData) {
        // TODO:  get id from vehicle
        let context = self.context.lock().await;
        let saved = context.repository.upsert("vehicle_altitude_data", &altitude_data).await;
        if let Err(err) = saved {
            println!("Save vehicle altitude data error: {:?}", &err);
        }

        self.mav_altitude_data.insert(mav_id, altitude_data);
    }
    ...
    async fn set_sns_data_for_mav(&mut self, mav_id: u8, sns_data: SnsData) {
        // TODO:  get id from vehicle
        let context = self.context.lock().await;
